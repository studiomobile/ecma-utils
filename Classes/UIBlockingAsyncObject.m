...
- (id)ifSuccess:(SEL)successSelector ifError:(SEL)errorSelector target:(id)target
      blockView:(UIView*)v andDisplayIndicator:(UIActivityIndicatorView*)i {
    checkNotNil(v, @"View cannot be null");
    return [self ifSuccess:successSelector
                   ifError:errorSelector
                    target:target
                blockViews:[NSArray arrayWithObject:v]
                 andDisplayIndicator:i];
}
